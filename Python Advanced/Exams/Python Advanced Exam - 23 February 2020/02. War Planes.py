...
for _ in range(n):
    if not any("t" in i for i in matrix):
    ...
    new_row = plane_pos[0] + (dirs[direction][0] * steps)
    new_col = plane_pos[1] + (dirs[direction][1] * steps)
...
left_targets = 0
if any("t" in i for i in matrix):
    for row in range(size):
        for col in range(size):
            if matrix[row][col] == "t":
                left_targets += 1
    print(f"Mission failed! {left_targets} targets left.")
elif not any("t" in i for i in matrix) and check:
    print(f"Mission accomplished! All {targets_destroyed} targets destroyed.")
