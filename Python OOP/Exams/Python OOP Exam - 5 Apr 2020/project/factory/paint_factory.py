...
class PaintFactory(Factory):
    ...
    def add_ingredient(self, ingredient_type, quantity):
        if ingredient_type in ["white", "yellow", "blue", "green", "red"] and quantity <= self.capacity:
            if ingredient_type not in self.ingredients:
                self.ingredients[ingredient_type] = 0
            self.ingredients[ingredient_type] += quantity
            self.capacity -= quantity
        elif ingredient_type in ["white", "yellow", "blue", "green", "red"] and quantity > self.capacity:
